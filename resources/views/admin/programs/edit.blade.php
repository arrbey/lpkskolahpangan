...
<div class="space-y-6 max-w-2xl">
    <!-- Header -->
    <div>
        <h2 class="text-2xl font-bold text-slate-800 font-heading">Edit Program Pelatihan</h2>
        <p class="text-sm text-slate-500">Ubah informasi program pelatihan.</p>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6">
        <form action="{{ route('admin.programs.update', $program->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="title" id="title" required value="{{ old('title', $program->title) }}" class="block w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-primary-green focus:border-transparent focus:outline-none transition">
            </div>

            <div>
                <label for="duration" class="block text-sm font-semibold text-slate-700 mb-2">Durasi Pelatihan</label>
                <input type="text" name="duration" id="duration" required value="{{ old('duration', $program->duration) }}" class="block w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-primary-green focus:border-transparent focus:outline-none transition">
            </div>

            <div>
                <label for="description" class="block text-sm font-semibold text-slate-700 mb-2">Deskripsi Lengkap Program</label>
                <textarea name="description" id="description" rows="4" required class="block w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-primary-green focus:border-transparent focus:outline-none transition">{{ old('description', $program->description) }}</textarea>
            </div>
            ...
            <div>
                ...
                @if($program->image)
                    <div class="mt-3 p-2 border border-slate-100 bg-slate-50 rounded-xl inline-block">
                        <img src="{{ Storage::disk('s3')->url($program->image) }}" alt="Program image" class="h-24 w-auto rounded-lg">
                    </div>
                @endif
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="order" id="order" required value="{{ old('order', $program->order) }}" class="block w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-primary-green focus:border-transparent focus:outline-none transition">
                </div>
